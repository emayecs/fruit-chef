 ...
 * @param aux
 */
void free_aux(aux_t *aux);

/*
TODO
*/
list_t *get_aux_bodies(aux_t *aux);

/**
 * true if did collide last tick, false if not
 *
 * @param aux
 *
 * @return bool
 */
bool get_aux_collide_last_tick(aux_t *aux);

/**
 * sets colliding_last_tick to specified vool
 *
 * @param aux
 *
 * @param bool
 *
 */
void set_aux_collide_last_tick(aux_t *aux, bool boolean);
...
typedef void (*collision_handler_t)(body_t *body1, body_t *body2, vector_t axis,
                                    void *aux);
// i deleted the parameter of the axis
